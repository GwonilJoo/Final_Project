...
<script>
    // 궈닐이 코드
    var loss_data = [['iter', 'Train Loss', 'Val loss']];
    var acc_data = [['iter', 'Train Acc', 'Val Acc']];
    var loss_iter = 1;
    var acc_iter = 1;

    function drawChart(train_data, val_data, chart_name) {
        var data, line_color;

        if(chart_name == "loss_chart"){
            loss_data.push([loss_iter, train_data, val_data]);
            loss_iter += 1;
            data = google.visualization.arrayToDataTable(loss_data);
            line_color = "blue";
        }
        else{
            acc_data.push([acc_iter, train_data, val_data]);
            acc_iter += 1;
    ...
    function drawInitChart(chart_name) {
        ...
        var loss_null_data = [['iter', 'Train Loss', 'Val Loss'], [0, 0, 0]];
        var acc_null_data = [['iter', 'Train Acc', 'Val Acc'], [0, 0, 0]];
...
<script type="text/javascript">
    ...
    webSocket.onmessage = function(message){
        ...
        if(dict.type == "chart"){
            ...
            google.charts.setOnLoadCallback(function(){
                drawChart(dict.train_loss, dict.val_loss, "loss_chart");
            });
            google.charts.setOnLoadCallback(function(){
                drawChart(dict.train_acc, dict.val_acc, "acc_chart");
            });
